<template>
  ...
  <Transition :name="name" @after-leave="done">
    ...
    >
      <li v-for="link in links" :key="link.path" class="border-b border-slate-500 last:border-none">
        <a
          :href="link.path"
          class="flex px-4 py-1.5 gap-2 before:border-l-4 before:border-transparent"
          :class="{
            'before:border-l-4 before:border-y-4 before:border-l-white before:h-0 before:self-center':
              isActive(link.path),
          }"
        >
          {{ link.title }}
